  ...
  <q-tr no-hover>
    ...
    <q-td>
      <q-chip  :ripple="false" v-if="message.amountValue" style="padding:0 0; margin: 0 0">
        <q-chip clickable @click="convertMoney" :ripple="false" :style="{'background-color': `#${parseInt(message.bgColor).toString(16).substring(2)}`}">
...
export default defineComponent({
  ...
  methods: {
    ...
    convertMoney() {
      try {
        window.open(`https://google.com/search?q=${this.message.amountValue} ${this.message.currency} in USD`)
