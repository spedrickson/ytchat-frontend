  ...
  <q-layout class="main-layout" view="hHh lpR fFf" style="display: flex">
    ...
    <q-page-container style="flex-grow: 1; display: flex">
      <q-list class="index-list row">
        <q-btn padding="sm" push color="primary" to="/filtered" class="index-btn col-12" icon="fas fa-filter">Filtered Chat</q-btn>
        <q-btn push color="primary" to="/sentiment" class="index-btn col-12" icon="fas fa-theater-masks">
          Chat Sentiment<br>
          <q-btn-group class="col-12 self-end" push style="text-align: center ">
            <q-btn color="light-blue-8" push to="/sentiment?m=yes&m=no" class="index-btn">Yes/No</q-btn>
            <q-separator vertical />
            <q-btn color="light-blue-8" class="sub-btn" padding="md" push label="0-" disable style="flex-shrink: 1; flex-grow: 0"/>
            <template v-for="index in 9" :key="index">
              <q-btn color="light-blue-8" class="sub-btn" padding="md" push :label="index+1" :to="sentimentCount(index+1)"/>
            </template>
          </q-btn-group>
        </q-btn>
        <q-btn push color="primary" to="/randommessage?test=true" class="index-btn col-12" icon="fas fa-dice">Random Message</q-btn>
      </q-list>
    </q-page-container>
  </q-layout>
